...
async def _level(
    ...
    concurrency: int, requests: int, server_pid: int | None,
) -> dict[str, Any]:
        ...
        async with semaphore:
            ...
            try:
                ...
                if response.headers.get("content-type", "").startswith("application/json"):
                    decisions[str(response.json().get("decision", "none"))] += 1
...
async def run(
    ...
    return {
        ...
        "limitations": [
            "Client-observed latency includes HTTP, authentication, policy and database work but excludes connector execution because /v1/evaluate is used.",
...
def main() -> None:
    ...
    report = asyncio.run(run(args.base_url, args.admin_token, levels, args.requests_per_level, args.server_pid))
