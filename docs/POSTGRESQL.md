...
v0.10 accepts `SENTINEL_DATABASE_URL=postgresql://...` and uses a bounded psycopg connection pool.
...
Approval-notification jobs use row-level `FOR UPDATE SKIP LOCKED` claims so multiple workers do
not own the same job. CI runs the disposable PostgreSQL race suite in `tests/integration`.

See [`OPERATIONS.md`](OPERATIONS.md) for backup creation, digest/catalog verification and the
bounded restore-drill command.
